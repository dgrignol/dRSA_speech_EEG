function [EEG, maskFromCleanRawFun] = preprocess_0p01to20Hz(EEG, maskFile)
% PREPROCESS_0P01TO20HZ Apply a 0.01-20 Hz band-pass to a prefilt EEG dataset.
%
...
%
% Output:
%   EEG                  - EEGLAB structure after 0.01-20 Hz filtering and trimming.
...
% `pop_eegfiltnew` so the time axis matches the original audio.
    ...
    % Apply 0.01-20 Hz band-pass filter
    originalLen = size(EEG.data, 2);
    EEG = pop_eegfiltnew(EEG, 0.01, 20, [], 0, [], 0);
